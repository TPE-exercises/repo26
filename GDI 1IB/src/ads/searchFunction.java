...
public class searchFunction {
	...
	public static void main(String[] args) {
		...
		searchNumber = readInt();
			...
			result3 = BinarySearchRecursive(numberSequence, searchNumber);
	...
	static int BinarySearchRecursive (int[] numberSequence, int searchNumber){
		 ...
		 if (numberSequence[m] == searchNumber)
			 ...
			 o = m-1;
			 int[] newNumberSequence = new int[(numberSequence.length - (m+1))];
			 //Inhalt des Arrays felt
			 BinarySearchRecursive(newNumberSequence, searchNumber);
		 }
		 else{
			 u = m+1;
			 int[] newNumberSequence = new int[(0 + (m+1))];
			//Inhalt des Arrays felt
			 BinarySearchRecursive(newNumberSequence, searchNumber);
		 }
