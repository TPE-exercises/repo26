...
public class BenTest {
	...
	private static BenTest bigInt = new BenTest(number);
	private static BenTest[] saveBigInts = new BenTest[100];
	
	BenTest(String bigInt) {
		number = bigInt;
		int numberCount = bigIntCount++;
	}
	...
	public static void main(String[] args) {
		...
		do {
			...
			if (bigIntAsString.length() > 0)
				saveBigInts[bigIntCount] = new BenTest(bigIntAsString);
			else
			...
			// Wandle String in Array um
			 arrayOfBigInt = BenTest.bigInt.getDigits(bigIntCount);
	...
	int[] getDigits(int bigIntCount) {
		...
		for (int i = currentLengthOfBigInt - 1; i >= 0; i--) {
			...
			char k = BenTest.number.charAt(i);
	...
	void length(int bigIntCount) {
		...
		currentLengthOfBigInt = BenTest.number.length();
